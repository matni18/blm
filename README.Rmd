...
The definition of a linear model is any model that takes on the form $Y = w_0 +\sum_{i=1}^n w_i\cdot f(x_i)$, for random variables $x_i \in [1;n]$ where $w_i$ respresents the weight associated with the term $f(x_i)$. In general, the function $f(x_i)$ can be any function, but this package only deals with cases where $f(x_i)$ is a linear function.
