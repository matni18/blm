...
The linear model must be built using a training dataset that contains one or more explanatory variables and a response variable. The `blm` constructor requires a prior distribution of weights (i.e. their mean and their variance). This can be any distribution, but the `blm` package provides a function `make_prior` that can construct this for you, based on a given prior precision, $\alpha$. This will output a covariance matrix of the form, $$\sigma_{ij} = \begin{cases} 1/\alpha,&\text{if i=j}\\0&\text{otherwise}\end{cases}$$
and a mean of 0 for all $w_i$.
Given a model, a prior distribution, a posterior precision $\beta$, and some data, `blm` then calculates a posterior distribution of the weights. This is a normal distribution with $w_i \sim N(m_{x,y},\sigma_{x,y})$ where $$\mathbf{m}_{x,y}=\beta \:\mathbf{S}_{x,y} {\phi_X}^T \mathbf{y},$$
...
Below is shown the results for a simulated data set as well as a prediction for a new data set.
```{r simulateTraining}
